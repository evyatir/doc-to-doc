// Book a Consultation. Per the brief: an emotional hook up top with one
// button that opens ("the toggle for consultation") the actual intake form
// + calendar below — not two separate pages, one page that reveals itself.
//
// Two independent things happen once it's open:
...
import { BRAND, KEYS, BOOK } from '@client/config';
...
export default function BookConsultation() {
  ...
  const [values, setValues] = useState({ name: '', email: '', role: '', stage: '', message: '', hp: '' });
  ...
  const submit = async () => {
    ...
    // auto-fill every input do. Silently "succeed" without posting.
    if (values.hp) { setDone(true); return; }
    ...
    try {
      ...
      if (isBackendAbsent(err)) {
        ...
        toast("You're going a bit fast — give it a minute and try again.");
      } else {
  ...
  return (
    <main className="book-page">
      <section className="book-hook">
        <p className="book-hero-note">we've been there. we'll walk with you.</p>
        <h1 className="page-h1">
          {BOOK.headline.map((line, i) => <span key={i}>{line}<br /></span>)}
        </h1>
        <p className="book-hook-sub">{BOOK.sub}</p>
        {!open && (
          <button className="btn btn-commerce book-hook-btn" onClick={revealForm}>
            {BOOK.cta}
          </button>
        )}
      </section>
      ...
      {open && (
        <div className="page book-reveal" ref={revealRef}>
          <div className="book-hero">
            <h2 className="page-h1 book-h1">Let's start with <span className="book-h1-accent">15 minutes.</span></h2>
            ...
            <div className="book-form-card">
              ...
              {done ? (
                ...
                <div className="form-grid" role="group" aria-label="Consultation intake form">
                  ...
                  {/* Honeypot — hidden from real visitors via CSS, not aria-hidden
                      (screen readers must still see it's not meant to be filled). */}
                  <div className="hp-field" aria-hidden="false">
                    <label htmlFor="bk-hp">Leave this field empty</label>
                    <input id="bk-hp" tabIndex="-1" autoComplete="off" value={values.hp} onChange={set('hp')} />
                  </div>
